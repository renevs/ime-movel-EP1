...
export class ProfessorService extends Ep1ApiService {
    ...
    getProfessor(): Promise<Professor[]> {
        return this.http.get(this.apiUrl + 'teacher').toPromise().then(this.extractData).catch(this.handleError);
    }

    searchProfessor(nusp: string): Promise<Professor> {
        return this.http.get(this.apiUrl + 'teacher/get/${nusp}').toPromise().then(this.extractData).catch(this.handleError);
    }
    ...
    updateProfessor(nusp: string, pass: string, name: string): Promise<any> {
        let body = 'nusp=' + nusp + '&pass=' + pass + '&name=' + name;
        return this.http.post(this.apiUrl + 'teacher/edit', body, this.options).toPromise().then(this.extractData).catch(this.handleError);
    }

    deleteProfessor(nusp: string): Promise<any> {
        let body = 'nusp=' + nusp;
        return this.http.post(this.apiUrl + 'teacher/delete', body, this.options).toPromise().then(this.extractData).catch(this.handleError);
    }  
}