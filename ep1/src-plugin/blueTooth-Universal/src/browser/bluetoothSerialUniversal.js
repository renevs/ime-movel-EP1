...
// Code from PhracturedBlue https://github.com/don/BluetoothSerialUniversal/issues/115
...
//
// BluetoothSerialUniversal.register(function(buf) {
//      //buf.input is the data that was received via BluetoothSerialUniversal.write
//      //buf.output is data that will be transmitted via a BluetoothSerialUniversal.read or subscribe
//      //Do processing here
//      buf.input = ""
// });
...
//   return buf;
// }
// BluetoothSerialUniversal.register(echoProxy);
...
module.exports = (function() {
    ...
    return {
        connect : function(mac, success_cb, fail_cb) {
            btlog("BluetoothSerialUniversal.connect: " + mac);
            connected = true;
        ...
        disconnect : function(macAddress, success_cb, fail_cb) {
            btlog("BluetoothSerialUniversal.disconnect");
            connected = false;
            window.clearInterval(interval);
            if (success_cb) { success_cb(); }
        },
        disconnectAll : function(success_cb, fail_cb) {
            btlog("BluetoothSerialUniversal.disconnectAll");
            connected = false;
            window.clearInterval(interval);
            if (success_cb) { success_cb(); }
        },
        write : function(data, success_cb, fail_cb) {
            btlog("BluetoothSerialUniversal.write: " + data);
            buf.input += data;
            if(success_cb) { success_cb(); }
        },
        available : function(success_cb, fail_cb) {
            btlog("BluetoothSerialUniversal.available");
            success_cb(buf.output.length);
        },
        read : function(success_cb, fail_cb) {
            btlog("BluetoothSerialUniversal.read: " + buf.output);
            var data = buf.output;
            buf.output = "";
            success_cb(data);
        },
        readUntil : function(delimiter, success_cb, fail_cb) {
            btlog("BluetoothSerialUniversal.readUntil");
        ...
        subscribe : function(success_cb, fail_cb) {
            btlog("BluetoothSerialUniversal.subscribe");
            subscribe_cb = success_cb;
            subscribe_delim = delimiter;
        },
        unsubscribe : function(success_cb, fail_cb) {
            btlog("BluetoothSerialUniversal.unsubscribe");
            subscribe_delim = false;
            if(success_cb) { success_cb(); }
        },
        subscribeRawData : function(success_cb, fail_cb) {
            btlog("BluetoothSerialUniversal.subscribeRawData");
            raw_cb = success_cb;
        },
        unsubscribeRawData : function(success_cb, fail_cb) {
            btlog("BluetoothSerialUniversal.unsubscribeRawData");
            raw_cb = false;
            if(success_cb) { success_cb(); }
        },
        clear : function(success_cb, fail_cb) {
            btlog("BluetoothSerialUniversal.clear");
            buf.output = "";
            if(success_cb) { success_cb(); }
        },
        ...
        listConnected : function(success_cb, fail_cb) {
          var devices = [{
              "class": 276,
              "id": "10:BF:48:CB:00:00",
              "address": "10:BF:48:CB:00:00",
        ...
        isConnected : function(macAddress, success_cb, fail_cb) {
            btlog("BluetoothSerialUniversal.isConnected: " + connected);
            if(connected) {
                if(success_cb) { success_cb(); }
            } else {
                if(fail_cb) { fail_cb(); }
        ...
        isEnabled : function(success_cb, fail_cb) {
            btlog("BluetoothSerialUniversal.isEnabled: " + enabled);
            if(enabled) {
                if(success_cb) { success_cb(); }
            } else {
                if(fail_cb) { fail_cb(); }
        ...
        readRSSI : function(success_cb, fail_cb) {
            alert("BluetoothSerialUniversal.readRSSI is not implemented");
        },
        showBluetoothSettings : function(success_cb, fail_cb) {
            alert("BluetoothSerialUniversal.showBluetoothSettings is not implemented");
        },
        enable : function(success_cb, fail_cb) {
            btlog("BluetoothSerialUniversal.enable");
            enable = true;
            if(success_cb) { success_cb(); }
        },
        discoverUnpaired : function(success_cb, fail_cb) {
            alert("BluetoothSerialUniversal.discoverUnpaired is not implemented");
        },
        startServer : function(success_cb, fail_cb) {
            btlog("BluetoothSerialUniversal.startServer" );
            connected = true;
            if (success_cb) { success_cb(); }
        },
